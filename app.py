...
def calculate():
    ...
    aggDict = pickle.load(open(mySiteDir+'aggDict.pkl', 'rb'))#Requires aggDict File
    chargeDict = pickle.load(open(mySiteDir+'chargeDict.pkl', 'rb'))#Requires chargeDict File
    #isoElP=[]
    netcharge=[]
    agg=[]
    chargeDensities=[]
    ...
    fMinMax=pd.read_csv(mySiteDir+"ampNonAMPminMaxesTable50.csv")
    for j in features:
        p=[]
        for i in p6.loc[:,j]:
    ...
    #newdf
    rfModel = pickle.load(open(mySiteDir+'rf_AMP_classifier_final_model.sav', 'rb'))
    ...
    if len(staphActCheckList) > 0:
        from propy import PyPro #Requires Installing Propy Package
        import propy #Requires Installing Propy Package
        from propy.PyPro import GetProDes #Requires Installing Propy Package
        import pickle
        labelDictS={"Neg":"Not Active","Plus":"Active"}
        ...
        for i in mr:
            take=np.where(mr == i)
            predS=eclf5Model.predict(newdf.iloc[take[0],1:].values)

        #if len(staphActCheckList) > 0:
        labelExp = labelDict[pred[0]] + "," + labelDictS[predS[0]] + " against S.aureus"
    else:
        labelExp = labelDict[pred[0]]

    #End Classifier#

    string_length = len(input_string)+5
    return render_template('index.html', label=labelExp, input_string=input_string)
